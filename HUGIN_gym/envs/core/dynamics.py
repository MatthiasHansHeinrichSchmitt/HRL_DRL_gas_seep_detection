...
class Dynamics:
    ...
    def step(self, state, action,conc):
        # -- Sample disturbances-- #
        ...
        #print(action)
        dx_local, dy_local, dz_local, dtheta = action # discrete action steps

        # Update state using existing physics model
        
        x, y, z, theta, concentration = (
            state["x"],
            state["y"],
            state["z"],
            state["theta"],
            state["concentration"]  # already in [0,1]
        )
        ...
        # Rotate local motion into world frame
        dx_world = dx_local * np.cos(theta) - dy_local * np.sin(theta)
        dy_world = dx_local * np.sin(theta) + dy_local * np.cos(theta)

        # Update world position
        state["x"] += round(dx_world)
        state["y"] += round(dy_world)
        state["z"] += round(dz_local)

        # Update orientation
        state["theta"] += dtheta
        #print(state["theta"])
        state["theta"] = (state["theta"]) % (2*np.pi) # keep theta in [0, 2pi)
        #print(state["theta"])
        # concentration update
        #state["concentration"] = conc[ix, iy, iz] # concentration is not updated here due to computational efficiency
        # --- continuous action space --- #
